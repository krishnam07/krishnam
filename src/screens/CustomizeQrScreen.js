...
import { buildUserQrPayload } from '../services/qrPayload';

const brandLogo = require('../assets/kd.png');

const VEHICLE_OPTIONS = ['Car', 'Bike', 'Scooter', 'Others'];
const SIZE_OPTIONS = ['Small', 'Medium', 'Large', 'Extra Large'];
const TAG_OPTIONS = [
  'Scan to connect',
  'Scan to contact owner',
  'Lost? Found? Scan me',
  'Lost? Parked Wrong? Scan Me',
  'Contact me',
  'Custom',
];
...
export default function CustomizeQrScreen({ navigation, route }) {
  ...
  const canConfirm = Boolean(vehicle && size && tagOption && (tagOption !== 'Custom' || customTag.trim()));
  ...
  async function confirm() {
    ...
    if (!canConfirm) {
      Alert.alert('Incomplete', 'Please complete QR type, dimensions and tagline.');
      return;
    }
    ...
    setConfirmed(true);
    setProcessing(false);
    Alert.alert('Confirmed', 'Customization applied. You can now download your QR.');
  }

  async function handleDownload() {
    if (!confirmed) {
      Alert.alert('Confirm first', 'Please confirm customization before downloading.');
      return;
    }
    ...
    try {
      const safeName = (user?.name || 'user').replace(/[^a-zA-Z0-9_-]/g, '_').toLowerCase();
      ...
      if (status === 'granted') {
        ...
        Alert.alert('Downloaded', 'Preview image saved to your phone gallery (Scanner Book album).');
        return;
      }
      ...
      Alert.alert('Download failed', err?.message || 'Unable to create image right now.');
    }
  }

  return (
    <ScrollView style={appStyles.container} contentContainerStyle={{ paddingBottom: 20 }}>
      <View style={appStyles.card}>
        <Text style={appStyles.title}>Customize your QR</Text>
        <Text style={[appStyles.subtitle, { marginTop: 8 }]}>Set QR type, dimensions and tagline before download.</Text>

        <View style={{ marginTop: 14 }}>
          <Text style={[appStyles.subtitle, { color: colors.text, fontWeight: '700', fontSize: 13 }]}>QR type</Text>
          <View style={{ borderWidth: 1, borderColor: '#2b4c73', borderRadius: 10, marginTop: 8, overflow: 'hidden' }}>
            <Picker
            ...
            >
              ...
              {VEHICLE_OPTIONS.map((option) => (
                <Picker.Item key={option} label={option} value={option} color={colors.text} />
              ))}
            </Picker>
          </View>
        </View>
        ...
        {vehicle ? (
          <View style={{ marginTop: 14 }}>
            <Text style={[appStyles.subtitle, { color: colors.text, fontWeight: '700', fontSize: 13 }]}>Add something interesting</Text>
            <View style={{ borderWidth: 1, borderColor: '#2b4c73', borderRadius: 10, marginTop: 8, overflow: 'hidden' }}>
              <Picker
                ...
                style={{ color: colors.text, backgroundColor: '#0c1a2d' }}
              >
                <Picker.Item label="Select an option" value="" color="#8ea9c7" />
        ...
        {tagOption === 'Custom' ? (
          <View style={{ marginTop: 12 }}>
            <Text style={[appStyles.subtitle, { color: colors.text, fontWeight: '700', fontSize: 13 }]}>Your custom tagline</Text>
            <TextInput
        ...
        {tagOption ? (
          <View style={{ marginTop: 14 }}>
            <Text style={[appStyles.subtitle, { color: colors.text, fontWeight: '700', fontSize: 13 }]}>Dimensions</Text>
            <View style={{ borderWidth: 1, borderColor: '#2b4c73', borderRadius: 10, marginTop: 8, overflow: 'hidden' }}>
              <Picker
                selectedValue={size}
                ...
                style={{ color: colors.text, backgroundColor: '#0c1a2d' }}
              >
                <Picker.Item label="Select dimensions" value="" color="#8ea9c7" />
        ...
        <View style={[appStyles.row, { marginTop: 18 }]}>
          <Pressable
            style={[appStyles.button, { flex: 1, backgroundColor: colors.success, opacity: processing ? 0.7 : 1 }]}
            onPress={confirm}
            disabled={processing}
          >
          ...
          <Pressable style={[appStyles.button, { flex: 1, backgroundColor: '#334155' }]} onPress={() => navigation.goBack()}>
      ...
      <View style={[appStyles.card, { alignItems: 'center' }]}>
        <Text style={[appStyles.subtitle, { marginBottom: 10, fontSize: 13 }]}>QR Preview</Text>
        {hasBaseQr && confirmed ? (
          <View
            ref={previewRef}
            collapsable={false}
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <View style={{ flex: 1, marginLeft: taglineMarginLeft }}>
                <Text style={{ color: '#102a43', fontSize: taglineFontSize, fontWeight: '800', lineHeight: taglineLineHeight }}>
                  {finalTagline || 'Scan to connect'}
                </Text>
                <Text style={{ color: '#0f1724', marginTop: 8, fontSize: 14, fontWeight: '700' }}>Scanner Book</Text>
                <Text style={{ color: '#365a7a', marginTop: 4, fontSize: 12 }}>Scan. Connect. Help.</Text>
              </View>
            </View>
          </View>
        ) : hasBaseQr ? (
          <View style={{ width: '100%', borderRadius: 12, borderWidth: 1, borderColor: '#2b4c73', padding: 12 }}>
            <Text style={[appStyles.subtitle, { marginTop: 0 }]}>Select all options and tap Confirm to see the final QR preview with tagline.</Text>
          </View>
        ) : (
          <View style={{ width: '100%', borderRadius: 12, borderWidth: 1, borderColor: '#2b4c73', padding: 12 }}>
            <Text style={[appStyles.subtitle, { marginTop: 0 }]}>QR unavailable: full name and contact number are required.</Text>
          </View>
        )}
        ...
        <Pressable
          style={[
            appStyles.button,
            { marginTop: 14, width: '100%', backgroundColor: confirmed && hasBaseQr ? colors.primary : '#334155' },
          ]}
